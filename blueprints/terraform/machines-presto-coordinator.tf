resource "triton_machine" "presto_coordinator" {
  name    = "${var.project_name}-presto-coordinator"
  ...
  cns {
    services = ["${local.cns_service_presto_coordinator}"]
  }
...
resource "null_resource" "presto_coordinator_install" {
  depends_on = [
  ...
  connection {
    host        = "${triton_machine.presto_coordinator.primaryip}"
    user        = "root"
